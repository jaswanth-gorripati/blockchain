...
//var dyChannel = require('./app/create-configtx-channel.js')
...
app.use(expressJWT({
	...
	path: ['/users','/','/app.js','/views/register.html','/views/createArtifacts.html','/views/getDetails.html','/views/addEducation.html','/favicon.ico','/index.html']
}));
///////////////////////////////////////////////////////////////////////////////
//////////////////////////////// START SERVER /////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
...
app.get("/app.js",function(req,res){
	res.sendFile(__dirname+"/app/views/app.js")
})
/*app.get("/angular.min.js",function(req,res){
	res.sendFile(__dirname+"/app/views/angular.min.js")
})
app.get("/angular-route.min.js",function(req,res){
	res.sendFile(__dirname+"/app/views/angular-route.min.js")
})*/
app.get("/views/createArtifacts.html",function(req,res){
	res.sendFile(__dirname+"/app/views/views/createArtifacts.html")
})
app.get("/views/register.html",function(req,res){
	res.sendFile(__dirname+"/app/views/views/register.html")
})
app.get("/views/addEducation.html",function(req,res){
	res.sendFile(__dirname+"/app/views/views/addEducation.html")
})
app.get("/views/getDetails.html",function(req,res){
	res.sendFile(__dirname+"/app/views/views/getDetails.html")
})
...
app.post('/users', function(req, res) {
	...
	helper.userAuthentication(username, orgName,Password, true).then(function(response) {
		if (response && typeof response !== 'string') {
			response.token = token;
			res.json(response);
		} else {
...
app.post('/channels', function(req, res) {
	...
	jwt.verify(token, app.get('secret'), function(err, decoded) {
		if (err) {
			...
			//if(channelName=="mychannel" || channelName == "myc"){
				channels.createChannel(channelName, channelConfigPath, decoded.username, decoded.orgName)
				.then(function(message) {
					res.send(message);
				});
			/*}else{
				dyChannel.createDynoChannel(channelName, channelConfigPath, decoded.username, decoded.orgName)
				.then(function(message) {
					res.send(message);
				});*/
			//}
...
app.post('/channels/:channelName', function(req, res) {
	...
	jwt.verify(token, app.get('secret'), function(err, decoded) {
		if (err) {
			...
			logger.debug('User name : ' + decoded.username);
			logger.debug('Org name  : ' + decoded.orgName);
			channels.getPeers(decoded.username, decoded.orgName).then(
				function(message) {
					res.send(message);
				});
		}
...
app.post('/channels/:channelName/chaincodes', function(req, res) {
	...
	jwt.verify(token, app.get('secret'), function(err, decoded) {
		if (err) {
			...
			logger.debug('User name : ' + decoded.username);
			logger.debug('Org name  : ' + decoded.orgName);
			instantiate.instantiateChaincode( channelName, chaincodeName,chaincodeVersion, functionName, args, decoded.username, decoded.orgName)
			.then(function(message) {
				res.send(message);
			});
		}
...
app.post('/channels/:channelName/chaincodes/:chaincodeName', function(req, res) {
	...
	var channelName = req.params.channelName;
	var args = req.body.args;
	var fcnName = req.body.functionName;
	logger.debug('channelName  : ' + channelName);
	logger.debug('peers : ' + peers); // target peers list
	logger.debug('chaincodeName : ' + chaincodeName);
	logger.debug('chaincodeVersion  : ' + chaincodeVersion);
	...
	jwt.verify(token, app.get('secret'), function(err, decoded) {
		if (err) {
			...
			logger.debug('User name : ' + decoded.username);
			logger.debug('Org name  : ' + decoded.orgName);
			let response = invoke.invokeChaincode(peers, channelName, chaincodeName,fcnName, args, decoded.username, decoded.orgName);
			response.then(function(resp) {
				/*if(fcnName=="getHistory"){
					if(resp[resp.length-1].StudentDetails.){
