...
var createChannel = function(channelName, channelConfigPath, username, orgName) {
	...
	helper.addChannel(channelName,orgName)
...
var getPeers = function(username,orgName) {
	logger.debug('\n====== Peer Details======\n');
	//helper.setupOrderer();
	var client = helper.getClientForOrg(orgName);
	var chain = helper.getChannelForOrg(orgName);
	//Acting as a client in the given organization provided with "orgName" param
	return helper.getRegisteredUsers(username, orgName).then((member) => {
		...
		logger.error('Failed to get peers ' + err.stack ? err.stack :
			err);
		throw new Error('Failed to get peers' + err.stack ? err.stack : err);
	});
};
