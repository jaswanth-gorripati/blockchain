...
function addChannel(channelName,org){
	let client = new hfc();
...
for (let key in ORGS) {
	if (key.indexOf('org') === 0) {
		...
		//for (var i = 0; i < config.channelName.length; i++) {
			...
			caClients[key] = new copService(caUrl, null /*defautl TLS opts*/,'' /* default CA*/, cryptoSuite);
				/*console.log("channels ---------->",channels[key])
				console.log("clients ---------->",clients[key])
				console.log("caClients -------------->",caClients[key])*/
...
// APIs
//-------------------------------------//
var getChannelForOrg = function(org,index) {
	if(index==undefined){
		return channels[org];
	}
	if(index!=undefined){
		return channels[org][index];
	}
	
};

var getClientForOrg = function(org,index) {
	if(index==undefined){
		return clients[org];
	}
	if(index!=undefined){
		return clients[org][index];
	}
...
var userAuthentication = function(userName,orgName,secret,isJson){
