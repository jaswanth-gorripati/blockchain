			...
			if (user && user.isEnrolled()) {
				...
				return getAdminUser(userOrg).then(function(adminUserObj) {
					...
					return cop.register({
						...
						role: 'client',
						attrs: [{"hf.Revoker":true},{"hf.Registrar.Roles":"client,user"},{"worksAs":"student"},{"worksAt":"little angels"}]}, member);
				}).then((secret) => {
					enrollmentSecret = secret;
					logger.debug(username + ' registered successfully with password:'+enrollmentSecret+'');
					return cop.enroll({
						enrollmentID: username,
						enrollmentSecret: secret
					});
